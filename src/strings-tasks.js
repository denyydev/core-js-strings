 ...
 *   repeatString('abc', -2) => ''
 */
function repeatString(letter, count) {
  return count <= 0 ? '' : letter.repeat(count);
}
...
function sumOfCodes(str) {
  return str ? str.split('').reduce((acc, n) => acc + n.charCodeAt(0), 0) : 0;
}
...
function countVowels(str) {
  ...
  return vowels.reduce((acc, n) => {
    const matches = str.match(new RegExp(n, 'g'));
...
function isPalindrome(str) {
  const clearSymbols = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
  return clearSymbols === clearSymbols.split('').reverse().join('');
}
...
function invertCase(str) {
  return str
    .split('')
    .map((n) => (n === n.toLowerCase() ? n.toUpperCase() : n.toLowerCase()))
    .join('');
}
...
function extractNameFromTemplate(value) {
  return value.replace('!', '').split(' ').slice(1, 3).join(' ');
}
...
function extractEmails(str) {
  ...
  return str.split(';').filter((string) => string.match(emailPattern));
}
...
function encodeToRot13(str) {
  ...
  const rotMapping = base.reduce((acc, n, index) => {
    acc[n] = rot[index];
    return acc;
  }, {});
  ...
  return str
    .split('')
    .map((n) => (rotMapping[n] === undefined ? n : rotMapping[n]))
    .join('');
}
...
function getCardId(value) {
  ...
  const cardsMapping = cards.reduce((acc, n, index) => {
    acc[n] = index;
    return acc;
  }, {});

  return cardsMapping[value];
}
